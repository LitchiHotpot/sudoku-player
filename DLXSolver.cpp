...
void DLXSolver:: solveWithAllAnswers(DLXNode *listHead, vector<CommonNode*>& tempSolution, vector<vector<CommonNode*>>& lastSolution, int depth) {
    ...
    for (DLXNode* node = columnHead->downNode; node != columnHead; node = node->downNode) {
        ...
        depth++;
        solveWithAllAnswers(listHead, tempSolution, lastSolution, depth); //Enter next recursion level
        depth--;
...
void DLXSolver:: cover(ColumnHead* columnHead) {
    ...
    for (DLXNode* tempNode = columnHead->downNode; tempNode != columnHead ; tempNode = tempNode->downNode) {
        for (DLXNode* tempNode2 = tempNode->rightNode;  tempNode2 != tempNode; tempNode2 = tempNode2->rightNode) {
...
void DLXSolver:: uncover(ColumnHead* columnHead) {
    for (DLXNode* tempNode = columnHead->upNode; tempNode != columnHead; tempNode = tempNode->upNode) {
        for (DLXNode* tempNode2 = tempNode->leftNode; tempNode2 != tempNode; tempNode2 = tempNode2->leftNode) {
            ((CommonNode*)tempNode2)->columnHead->numberOfOne++;
...
ColumnHead* DLXSolver:: selectColumn(DLXNode* listHead){
    ...
    for (DLXNode* node = listHead->rightNode; node != listHead; node = node->rightNode){
